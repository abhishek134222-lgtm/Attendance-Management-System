async function loadDashboard() {
  try {
    ...
    if (attendance.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:rgba(255,255,255,0.4); padding:30px;">No attendance marked today yet</td></tr>';
      return;
    }
    ...
    attendance.forEach(row => {
      ...
      tr.innerHTML = `<td>${row.roll_no}</td><td>${row.name}</td><td>${row.class}</td><td style="color:${color}; font-weight:600;">${row.status}</td>`;
